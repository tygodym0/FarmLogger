...
local NFT_item = {
    [1] = {'Золотая рулетка', 'gold_r',},
    [2] = {'Бензопила на спину'},
    [3] = {'Серебряная рулетка'},
    [4] = {'Серебро'},
    [5] = {'Золото'},
    [6] = {'Осколок предмета заточка +13'},
    [7] = {'Талон AZ Coins'},
    [8] = {'Сертификат Elegy'},
    [9] = {'Сертификат Cheetah'},
    [10] = {'Сертификат Картинг'},
    [11] = {'Сертификат Phoenix'},
    [12] = {'Набор реставрации для акссесуара'}
    -- Пачка с деньгами
    -- Талон AZ Coins
    -- Золотая рулетка [1]
                ...
                if imgui.BeginPopupModal(u8'Настройки', _, imgui.WindowFlags.NoResize) then
                    if imgui.BeginTabBar('Tabs') then -- задаём начало вкладок
                        ...
                        if imgui.BeginTabItem(u8'Input вкладка') then -- вторая вкладка
                            --[[for k = 1, #item, 2 do
                                if item[k] and item[k][5] and item[k][6] and item[k][7] and item[k+1] and item[k+1][5] and item[k+1][6] and item[k+1][7] then
                                    imgui.Text(u8(item[k][5]))
                                    if item[k+1] and item[k+1][5] then
                                        imgui.SameLine(0, 35)
                                    ...
                                    if item[k+1] and item[k+1][5] and item[k+1][6] and item[k+1][7] then
                                        local settingsInputsWidth = 175
                                        imgui.SameLine(0, 35)
                                        imgui.SetNextItemWidth(settingsInputsWidth)
                                        --imgui.SameLine(0, 35)
                                ...
                                if item[k][1] and item[k][7] and (item[k][8]) then
                                    if item[k][8][0] then
                                        if FarmData.search_filter:PassFilter(u8(item[k][1])) then
                                            --imgui.Text(u8('ПРЕДМЕТ: ' .. item[k][1] .. ' | ЦЕНА: ' .. number_separator(item[k][7][0])))
                            ...
                            if imgui.InputInt(u8('##priceInput'), FarmData.input_price, 0, 0) then
                                if not (FarmData.current_editing == '') then
                                    --FarmData.current_editing = tonumber(FarmData.input_price[0])
...
function main()
    ...
    sampRegisterChatCommand('fsave', function() -- СЕЙВ ПОСЛЕ КАКИХ ЛИБО НОВЫХ ПЕРЕМЕННЫХ ИЛИ ИЗМЕНЕНИЙ
        ...
        local buffer_gold_r_second = FarmLog.weeks[currentWeek].gold_r_second or 0
        local buffer_benzopila_na_spiny = FarmLog.weeks[currentWeek].benzopila_na_spiny or 0
        local buffer_silver_r_second = FarmLog.weeks[currentWeek].silver_r_second or 0
        local buffer_silver = FarmLog.weeks[currentWeek].silver or 0
        local buffer_gold = FarmLog.weeks[currentWeek].gold or 0
        local buffer_oskolok_zatochka_nft = FarmLog.weeks[currentWeek].oskolok_zatochka_nft or 0
        local buffer_az_second = FarmLog.weeks[currentWeek].az_second or 0
        local buffer_nft_sert_elegy = FarmLog.weeks[currentWeek].nft_sert_elegy or 0
        local buffer_nft_sert_cheetah = FarmLog.weeks[currentWeek].nft_sert_cheetah or 0
        local buffer_nft_sert_carting = FarmLog.weeks[currentWeek].nft_sert_carting or 0
        local buffer_nft_sert_phoenix = FarmLog.weeks[currentWeek].nft_sert_phoenix or 0
        local buffer_nft_restavracia_acs = FarmLog.weeks[currentWeek].nft_restavracia_acs or 0
        local buffer_nft_sa_money = FarmLog.weeks[currentWeek].nft_sa_money or 0

        FarmLog.weeks[currentWeek] = {current_editing = '', sa = buffer_sa or 0, az = buffer_az or 0, vc = buffer_vc or 0, nft_sa_money = buffer_nft_sa_money or 0, nft_restavracia_acs = buffer_nft_restavracia_acs or 0, nft_sert_phoenix = buffer_nft_sert_phoenix or 0, nft_sert_carting = buffer_nft_sert_carting or 0, nft_sert_cheetah = buffer_nft_sert_cheetah or 0, nft_sert_elegy = buffer_nft_sert_elegy or 0, az_second = buffer_az_second or 0, oskolok_zatochka_nft = buffer_oskolok_zatochka_nft or 0, gold = buffer_gold or 0, silver = buffer_silver or 0, silver_r_second = buffer_silver_r_second or 0, benzopila_na_spiny = buffer_benzopila_na_spiny or 0, gold_r_second = buffer_gold_r_second or 0, micro_tec = buffer_micro_tec or 0, space_heart = buffer_space_heart or 0, altushka = buffer_altushka or 0, finka_lv_territory = buffer_finka_lv_territory or 0, quest_business_sa = buffer_quest_business_sa or 0, quest_business_az = buffer_quest_business_az or 0, finka_business = buffer_finka_business or 0, primogem = buffer_primogem or 0, second_hand_box = buffer_second_hand_box or 0, minecraft_box = buffer_minecraft_box or 0, gentleman_box = buffer_gentleman_box or 0, marvel_box = buffer_marvel_box or 0, super_moto_box = buffer_super_moto_box or 0, super_auto_box = buffer_super_auto_box or 0, rare_blue_box = buffer_rare_blue_box or 0, rare_red_box = buffer_rare_red_box or 0, rare_yellow_box = buffer_rare_yellow_box or 0, nostalgic_box = buffer_nostalgic_box or 0, fortnite_box = buffer_fortnite_box or 0, organization_box = buffer_organization_box or 0, oligarch_box = buffer_oligarch_box or 0, random_box = buffer_random_box or 0, mortal_combat_box = buffer_mortal_combat_box or 0, custom_accessories_box = buffer_custom_accessories_box or 0, crafter_box = buffer_crafter_box or 0, treasure_hunter_box = buffer_treasure_hunter_box or 0, fisher_box = buffer_fisher_box or 0, products_carrier_box = buffer_products_carrier_box or 0, total_boxes = buffer_total_boxes or 0, total_roulette = buffer_total_roulette or 0, total_business = buffer_total_business or 0, obrez = buffer_obrez or 0, kosa_marci = buffer_kosa_marci or 0, bitcoin = buffer_bitcoin or 0, midas3slot = buffer_midas3slot or 0, grazdan_taloni = buffer_grazdan_taloni or 0, concept_car_luxury_box = buffer_concept_car_luxury_box or 0, larec_premiya = buffer_larec_premiya or 0, super_car_box = buffer_super_car_box or 0, bronze_r = buffer_bronze_r or 0, silver_r = buffer_silver_r or 0, gold_r = buffer_gold_r or 0, platinum_r = buffer_platinum_r or 0, moneta_mirage = buffer_moneta_mirage or 0, leshiy = buffer_leshiy or 0, podarki_acs_ohr = buffer_podarki_acs_ohr or 0, az_acs_ohr = buffer_az_acs_ohr or 0, ribmoneta_acs_ohr = buffer_ribmoneta_acs_ohr or 0, vc_acs_ohr = buffer_vc_acs_ohr or 0, payday = buffer_payday or 0, zarplata = buffer_zarplata or 0, deposit = buffer_deposit or 0, mirage = buffer_mirage or 0, market = buffer_market or 0, rassrochka = buffer_rassrochka or 0, premiumvip = buffer_premiumvip or 0, premiumvipy = buffer_premiumvipy or 0, addvip = buffer_addvip or 0}
        ...
        local buffer_gold_r_second = FarmLog.months[currentMonth].gold_r_second or 0
        local buffer_benzopila_na_spiny = FarmLog.months[currentMonth].benzopila_na_spiny or 0
        local buffer_silver_r_second = FarmLog.months[currentMonth].silver_r_second or 0
        local buffer_silver = FarmLog.months[currentMonth].silver or 0
        local buffer_gold = FarmLog.months[currentMonth].gold or 0
        local buffer_oskolok_zatochka_nft = FarmLog.months[currentMonth].oskolok_zatochka_nft or 0
        local buffer_az_second = FarmLog.months[currentMonth].az_second or 0
        local buffer_nft_sert_elegy = FarmLog.months[currentMonth].nft_sert_elegy or 0
        local buffer_nft_sert_cheetah = FarmLog.months[currentMonth].nft_sert_cheetah or 0
        local buffer_nft_sert_carting = FarmLog.months[currentMonth].nft_sert_carting or 0
        local buffer_nft_sert_phoenix = FarmLog.months[currentMonth].nft_sert_phoenix or 0
        local buffer_nft_restavracia_acs = FarmLog.months[currentMonth].nft_restavracia_acs or 0
        local buffer_nft_sa_money = FarmLog.months[currentMonth].nft_sa_money or 0

        FarmLog.months[currentMonth] = {current_editing = '', sa = buffer_sa or 0, az = buffer_az or 0, vc = buffer_vc or 0, nft_sa_money = buffer_nft_sa_money or 0, nft_restavracia_acs = buffer_nft_restavracia_acs or 0, nft_sert_phoenix = buffer_nft_sert_phoenix or 0, nft_sert_carting = buffer_nft_sert_carting or 0, nft_sert_cheetah = buffer_nft_sert_cheetah or 0, nft_sert_elegy = buffer_nft_sert_elegy or 0, az_second = buffer_az_second or 0, oskolok_zatochka_nft = buffer_oskolok_zatochka_nft or 0, gold = buffer_gold or 0, silver = buffer_silver or 0, silver_r_second = buffer_silver_r_second or 0, benzopila_na_spiny = buffer_benzopila_na_spiny or 0, gold_r_second = buffer_gold_r_second or 0, micro_tec = buffer_micro_tec or 0, space_heart = buffer_space_heart or 0, altushka = buffer_altushka or 0, finka_lv_territory = buffer_finka_lv_territory or 0, quest_business_sa = buffer_quest_business_sa or 0, quest_business_az = buffer_quest_business_az or 0, finka_business = buffer_finka_business or 0, primogem = buffer_primogem or 0, second_hand_box = buffer_second_hand_box or 0, minecraft_box = buffer_minecraft_box or 0, gentleman_box = buffer_gentleman_box or 0, marvel_box = buffer_marvel_box or 0, super_moto_box = buffer_super_moto_box or 0, super_auto_box = buffer_super_auto_box or 0, rare_blue_box = buffer_rare_blue_box or 0, rare_red_box = buffer_rare_red_box or 0, rare_yellow_box = buffer_rare_yellow_box or 0, nostalgic_box = buffer_nostalgic_box or 0, fortnite_box = buffer_fortnite_box or 0, organization_box = buffer_organization_box or 0, oligarch_box = buffer_oligarch_box or 0, random_box = buffer_random_box or 0, mortal_combat_box = buffer_mortal_combat_box or 0, custom_accessories_box = buffer_custom_accessories_box or 0, crafter_box = buffer_crafter_box or 0, treasure_hunter_box = buffer_treasure_hunter_box or 0, fisher_box = buffer_fisher_box or 0, products_carrier_box = buffer_products_carrier_box or 0, total_boxes = buffer_total_boxes or 0, total_roulette = buffer_total_roulette or 0, total_business = buffer_total_business or 0, obrez = buffer_obrez or 0, kosa_marci = buffer_kosa_marci or 0, bitcoin = buffer_bitcoin or 0, midas3slot = buffer_midas3slot or 0, grazdan_taloni = buffer_grazdan_taloni or 0, concept_car_luxury_box = buffer_concept_car_luxury_box or 0, larec_premiya = buffer_larec_premiya or 0, super_car_box = buffer_super_car_box or 0, bronze_r = buffer_bronze_r or 0, silver_r = buffer_silver_r or 0, gold_r = buffer_gold_r or 0, platinum_r = buffer_platinum_r or 0, moneta_mirage = buffer_moneta_mirage or 0, leshiy = buffer_leshiy or 0, podarki_acs_ohr = buffer_podarki_acs_ohr or 0, az_acs_ohr = buffer_az_acs_ohr or 0, ribmoneta_acs_ohr = buffer_ribmoneta_acs_ohr or 0, vc_acs_ohr = buffer_vc_acs_ohr or 0, payday = buffer_payday or 0, zarplata = buffer_zarplata or 0, deposit = buffer_deposit or 0, mirage = buffer_mirage or 0, market = buffer_market or 0, rassrochka = buffer_rassrochka or 0, premiumvip = buffer_premiumvip or 0, premiumvipy = buffer_premiumvipy or 0, addvip = buffer_addvip or 0}
    ...
    sampRegisterChatCommand('check59', function()
        ...
        if eventCall == 'containerContent.initialize' then
            ...
            if dataCall:find('"items":%[') then
                ...
                if items_info then
                    sampAddChatMessage(items_info)
                    --lua_thread.create(function()
                        --wait(800) -- Добавляем wait, потому что в течение 10 миллисекунд отправляются еще 6 аналогичных пакетов. Мы должны обработать только 1
                        for itemm in items_info:gmatch('{([^}]+)}') do
                            local item_name = itemm:match('"title":"([^"]+)"')
                            local count_item = itemm:match('"count":(%d+)')
    ...
    sampRegisterChatCommand('check61', function()
        for i, v in pairs(item) do
            print(i, v[1])
        end
    end)

    sampRegisterChatCommand('check62', function()
        delete_all_loot(item[58][3])
    end)

    sampRegisterChatCommand('check63', function()
        sampAddChatMessage('НЫНЕШНЕЕ СОСТОЯНИЕ: ' .. (imguiJson.flag_nft and 'ВОРК' or 'НЕ ВОРК'), -1)
        imguiJson.flag_nft = not imguiJson.flag_nft
        --imguiJson.flag_nft()
        sampAddChatMessage('ЗАСЕЙВЛЕННОЕ СОСТОЯНИЕ: ' .. (imguiJson.flag_nft and 'ВОРК' or 'НЕ ВОРК'), -1)
        if imguiJson.flag_nft == false then
            sampAddChatMessage('ДА ПОШЕЛ ТЫ В ПИЗДУ FALSE')
        else
            sampAddChatMessage('ГАНДОН')
        end
    end)

    sampRegisterChatCommand('check64', function()
        --[[for k = 1, #item, 1 do
            if item[k] and item[k][5] and item[k][6] and item[k][7] and (item[k+1]) and (item[k+1][5]) and (item[k+1][6]) and (item[k+1][7]) then
                print(' [' .. k .. '] ' .. item[k][1])
            end
        ...
        --[[if item[45][1] and item[45][2] and item[45][3] and item[45][4] and item[45][5] and item[45][6] and item[45][7] then
            sampAddChatMessage('ДА', -1) -- ВЫВОД: ДА
        end]]

        --[[if item[46][1] and item[46][2] and item[46][3] and item[46][4] and item[46][5] and item[46][6] and item[46][7] then
            sampAddChatMessage('ДА', -1) -- ВЫВОД: НЕТ
        else
            sampAddChatMessage('НЕТ', -1)
        end]]

        --[[if item[46][5] == _ then
            sampAddChatMessage('НУ ДЯ', -1) -- ВЫВОД: НУ ДЯ
        else
            sampAddChatMessage('НЕ', -1)
        end]]

        for k = 1, #item, 2 do
            if item[k] and item[k][5] and item[k][6] and item[k][7] then
                print(' [' .. k .. '] ' .. item[k][1])
            elseif item[k] and (item[k][5] == _) or (item[k][6] == _) or (item[k][7] == _) then
                sampAddChatMessage('ХОБА, НАШЛИ ПАРУ', -1)
                k = k + 1
            end
        end
    end)

    sampRegisterChatCommand('check65', function()
        if FarmData.search_filter then
            sampAddChatMessage('да')
        else
            sampAddChatMessage('нет')
        end
    end)

    sampRegisterChatCommand('check66', function()
        if item[58][8] then
            if item[58][8][0] then
                sampAddChatMessage('Да', -1)
            end
    ...
    sampRegisterChatCommand('check67', function()
        local predmet = 'Лён'
    ...
    sampRegisterChatCommand('check68', function()
        print(sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(1))))
    end)

    sampRegisterChatCommand('check69', function()
        print(menu[17][4])
    end)
